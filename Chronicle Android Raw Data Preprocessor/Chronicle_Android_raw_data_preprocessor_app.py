...
class ChronicleAndroidRawDataPreprocessor:
    ...
    def apply_timezone_handling_options(self) -> None:
        ...
        final_row_count = len(self.current_participant_raw_data_df)
        LOGGER.info(f"Timezone handling completed. Final row count: {final_row_count}")
...
# class FilterCategoriesWindow(QDialog):
#     """
#     A dialog window for filtering categories of apps.

#     Attributes:
#         filter_categories_dict (dict[str, pd.DataFrame]): Dictionary containing categories and their corresponding DataFrames.
#         selected_apps (dict[str, list[str]]): Dictionary containing selected apps for each category.
#     """

#     def __init__(
#         self, parent_: ChronicleAndroidRawDataPreprocessorApp, filter_categories_dict: dict[str, pd.DataFrame], selected_apps: dict[str, list[str]]
#     ) -> None:
#         """
#         Initialize the FilterCategoriesWindow.

#         Args:
#             parent_ (ChronicleAndroidRawDataPreprocessorApp): The parent application.
#             filter_categories_dict (dict[str, pd.DataFrame]): Dictionary containing categories and their corresponding DataFrames.
#             selected_apps (dict[str, list[str]]): Dictionary containing selected apps for each category.
#         """
#         super().__init__(parent_)
#         self.filter_categories_dict = filter_categories_dict
...
#         self.table.setHorizontalHeaderLabels(["Category", "Number of Apps Total", "Number of Apps Selected to be Filtered", "Actions"])
...
#         """
#         Update the table with the current filter categories and selected apps.
#         """
#         self.table.setRowCount(len(self.filter_categories_dict))

#         for row, (category, df) in enumerate(self.filter_categories_dict.items()):
#             total_apps = len(df)
#             selected_apps = len(self.selected_apps.get(category, []))

#             category_item = QTableWidgetItem(category)
#             category_item.setTextAlignment(Qt.AlignCenter)
#             total_apps_item = QTableWidgetItem(str(total_apps))
#             total_apps_item.setTextAlignment(Qt.AlignCenter)
#             selected_apps_item = QTableWidgetItem(str(selected_apps))
#             selected_apps_item.setTextAlignment(Qt.AlignCenter)

#             self.table.setItem(row, 0, category_item)
#             self.table.setItem(row, 1, total_apps_item)
#             self.table.setItem(row, 2, selected_apps_item)

#             button = QPushButton("Filter Apps")
#             button.clicked.connect(lambda _, cat=category: self.open_filter_apps_window(cat))
...
#         for row in range(self.table.rowCount()):
#             table_height += self.table.rowHeight(row)
#         self.resize(min(table_width, 1000), min(table_height, 600))
#         logger.debug(f"{self.__class__.__name__} resized to fit table")

#     def open_filter_apps_window(self, category: str) -> None:
#         """
#         Open a window to filter apps for a specific category.

#         Args:
#             category (str): The category to filter apps for.
#         """
#         if FilterAppsWindow.instance is not None:
#             FilterAppsWindow.instance.raise_()
#             FilterAppsWindow.instance.activateWindow()
#             return
#         self.filter_apps_window = FilterAppsWindow(self, category, self.filter_categories_dict[category], self.selected_apps.get(category, []))
#         self.filter_apps_window.exec_()
#         self.update_table()


# class FilterAppsWindow(QDialog):
#     """
#     A dialog window for filtering apps within a specific category.

#     Attributes:
#         instance (FilterAppsWindow): Singleton instance of the FilterAppsWindow.
#         category (str): The category of apps being filtered.
#         df (pd.DataFrame): DataFrame containing app data for the category.
#         selected_apps (list[str]): List of selected apps for the category.
#     """

#     instance = None

#     def __init__(self, parent_: FilterCategoriesWindow, category: str, category_df: pd.DataFrame, selected_apps: list[str]) -> None:
#         """
#         Initialize the FilterAppsWindow.

#         Args:
#             parent_ (FilterCategoriesWindow): The parent window.
#             category (str): The category of apps being filtered.
#             df (pd.DataFrame): DataFrame containing app data for the category.
#             selected_apps (list[str]): List of selected apps for the category.
#         """
...
#         self.init_UI()
#         self._center_on_parent()
#         FilterAppsWindow.instance = self
...
#         Initialize the user interface of the dialog.
#         """
#         self.setWindowTitle(f"Filter Apps - {self.category}")
#         self.setGeometry(150, 150, 600, 400)
#         self.table = QTableWidget(self)
#         self.table.setColumnCount(3)
#         self.table.setHorizontalHeaderLabels(["App Package Name", "Application Label", "Filter?"])
#         self.table.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)
#         self.update_table()
#         layout = QVBoxLayout()
#         layout.addWidget(self.table)
...
#     def update_table(self) -> None:
#         """
#         Update the table with the current apps and their filter status.
#         """
...
#         # Populate the table
#         for row, (_, app) in enumerate(self.df.iterrows()):
#             package_name_item = QTableWidgetItem(app["app_package_name"])
#             package_name_item.setTextAlignment(Qt.AlignCenter)
#             label_item = QTableWidgetItem(app["application_label"])
#             label_item.setTextAlignment(Qt.AlignCenter)
#             self.table.setItem(row, 0, package_name_item)
#             self.table.setItem(row, 1, label_item)

#             checkbox = QCheckBox()
#             checkbox.setChecked(app["app_package_name"] in self.selected_apps)
#             checkbox.stateChanged.connect(lambda state, app_name=app["app_package_name"]: self.update_selected_apps(state, app_name))
...
#     def update_selected_apps(self, state: int, app_name: str) -> None:
#         """
#         Update the list of selected apps based on the checkbox state.

#         Args:
#             state (int): The state of the checkbox (checked or unchecked).
#             app_name (str): The name of the app being updated.
#         """
#         if state == Qt.Checked:
...
#         """
#         self.parent().selected_apps[self.category] = self.selected_apps
#         FilterAppsWindow.instance = None
